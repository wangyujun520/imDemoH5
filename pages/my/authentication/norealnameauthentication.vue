	...
	<view class="content">
		...
		<view class="bg-white px-3">
			<view class="flex align-center"
				style="height: 120rpx;border-bottom: 1px solid rgba(187,187,187,0.2);border-top: 1px solid rgba(187,187,187,0.2);">
				<view class="form-title" style="width: 80rpx;"><text style="font-size: 30rpx;color: #101010;">姓名</text>
				</view>
				<view class="form-input" style="width: 500rpx;">
					<text style="font-size: 28rpx;color: #101010;" v-if="Name == ''">自动识别</text>
					<input v-else maxlength="8" type="text" style="font-size: 28rpx;color: #777;" placeholder="请填写姓名"
						v-model="Name" />
				</view>
			</view>
			<view class="flex align-center" style="height: 120rpx;border-bottom: 1px solid rgba(187,187,187,0.2);">
				<view class="form-title" style="width: 80rpx;"><text style="font-size: 30rpx;color: #101010;">性别</text>
				</view>
				<view class="form-input" style="width: 570rpx;">
					<text style="font-size: 28rpx;color: #101010;" v-if="GenderShow == '' ">自动识别</text>
					<input v-else disabled="disabled" type="text" style="font-size: 28rpx;color: #777;"
						v-model="GenderShow" />
				</view>
			</view>
			<view class="flex align-center" style="height: 120rpx;border-bottom: 1px solid rgba(187,187,187,0.2);">
				<view class="form-title" style="width: 150rpx;"><text
						style="font-size: 30rpx;color: #101010;">证件类型</text></view>
				<view class="form-input" style="width: 500rpx;"><input type="text" disabled="disabled"
						v-model="CertificatesType" style="font-size: 28rpx;color: #444;" /></view>
			</view>
			<view class="flex align-center" style="height: 120rpx;border-bottom: 1px solid rgba(187,187,187,0.2);">
				<view class="form-title" style="width: 150rpx;"><text
						style="font-size: 30rpx;color: #101010;">身份证号</text></view>
				<view class="form-input" style="width: 500rpx;">
					<text style="font-size: 28rpx;color: #101010;" v-if="IDNumber == ''">自动识别</text>
					<input v-else type="number" maxlength="18" style="font-size: 28rpx;color: #777;"
						placeholder="请填写身份证号" v-model="IDNumber" />
				</view>
			</view>
		</view>
		<view class="flex flex-column">
			...
			<view class="flex justify-center">
				<view @click="PositiveUpload" class="rounded IdImg"
					:style="{ backgroundImage: 'url(' + IDCardPositive + ')' }">
					<view v-if="ShowPositive" class="flex flex-column justify-center align-center"
						style="width: 100%;height: 100%;">
						<image src="/static/img/ImageArchives/iv_idcard_add.png" style="width: 100rpx;height: 100rpx;">
						</image>
		...
		<view class="flex flex-column">
			...
			<view class="flex justify-center">
				<view @click="BackUpload" class="rounded IdImg" :style="{ backgroundImage: 'url(' + IDCardBack + ')' }">
					<view v-if="ShowBack" class="flex flex-column justify-center align-center"
						style="width: 100%;height: 100%;">
						<image src="/static/img/ImageArchives/iv_idcard_add.png" style="width: 100rpx;height: 100rpx;">
						</image>
		...
		<view class="flex align-center justify-center" style="height: 150rpx;">
			<u-button @click="Submit" type="primary"
				style="width: 640rpx;height: 80rpx;font-size: 26rpx;background-color: #FF9900;">提交</u-button>
...
<script>
	...
	import {
		pathToBase64,
		base64ToPath
	} from '@/common/imgBasetoPath/index.js'; //base64转换路径
	...
	export default {
		...
		methods: {
						...
						if (result.code == 200) {
							...
							base64ToPath(Positive).then(path => {
								...
								for (let k = 0; k < ImgSplit.length; k++) {
									...
									var FileName = year + '' + month + '' + day + '' + new Date().getTime() +
										k + '.' + extendName; //202003021583151162210.jpg
									var fuleFileName = 'RealNameAuthenticationInfo' + '/' + date
									.getFullYear() + '/' + month + '/' + FileName;
									console.log(fuleFileName);
									var SuccessUrlImg = OssUpload.fileUploadBack('image', imgSrc,
									fuleFileName);
			...
			Submit() {
				...
				var RequrestData = {
					gender: this.Gender, //性别；1=男；0=女
					name: this.Name, //姓名
					card_type: "1", //证件类型，默认为1=身份证
					id_card: this.IDNumber, //身份证号
					mobile: "", //手机号码
					card_front: this.IDCardPositive, //身份证正面图片
					card_back: this.IDCardBack //身份证背面图片
				}

				this.$post(this.$url.User.RealNameAuthentication, RequrestData, {
					isLoading: true,
					loadingTitle: '认证中...'
				}).then(res => {
					// console.log(123)
			...
			IMUpdate() {
				var MyInfo = {
					...
					gender: this.Gender == 0 ? getApp().$TIM.TYPES.GENDER_FEMALE : getApp().$TIM.TYPES
					.GENDER_MALE, //性别
					profileCustomField: [{
