	...
	<view class="px-2 pb-2">
		<view class="flex align-center justify-between bg-white rounded mt-2 px-2" style="height: 110rpx;" v-for="(item, index) in DataList">
			...
			<u-radio-group width="40rpx" size="38" v-model="value" @change="radioGroupChange">
				<u-radio @change="radioChange" :name="DataList[index].name" :disabled="DataList[index].disabled" active-color="#ff9900"
				 :icon-size="20">
				</u-radio>
			</u-radio-group>
	...
	export default {
		...
		methods: {
			Submit() {
				...
				if (typeof(prevPage.$vm.DataList) == "undefined") {
					
					prevPage.$vm.dataList.houseStyle = this.value;
