			...
			<view class="flex flex-column m-2">
				<view class="bg-white" style="width: 100%;border-radius: 10rpx 10rpx 0 0;">
					<view class="flex flex-column">
						...
						<text style="font-size: 30rpx;color: #02B1BE;font-weight: bold;">{{item.estate_title }}</text>
						
						<view class="flex align-center">
							<text>材料数量：{{item.count}}</text>
	...
	export default {
		...
		methods: {
			...
			skipOrderDetails(item) {
				...
				// 	url: '/pages/tabbar/message/SubPage/SalesCollaboration/OrderDetails?id='+item.id
				// })
				...
				if(item.type==1){
					uni.navigateTo({
						url:'/pages/tabbar/my/OrderWork/CreateOrder?id='+item.id
					})
				}
			...
			InitData() {
				...
				// 获取销售协同明细
				this.$post(this.$url.Sale.GetOrders, requestData, {
					isLoading: true,
					loadingTitle: '加载中...'
				}).then(res => {
