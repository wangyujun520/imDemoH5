	...
	export default {
		onNavigationBarButtonTap: function(e) {
			if (e.float == 'right') {
				...
				let requestData = {
					...
					catalog_id: this.catalog_id //施工类别id
				}
				// 增加项目工人
				this.$post(this.$url.Work.AddWorker, requestData, {
					isLoading: false
				}).then(res => {
					uni.navigateBack({
						delta: 1
		...
		methods: {
			...
			InitData() {
				this.SelectValue = [];
				// 获取工人明细
				this.$post(this.$url.Work.GetWorkers, {
					// key: "", //关键字
					catalog: this.tabsList[this.current].id //类别；工人类别字典
				}, {
					isLoading: false
				}).then(res => {
					res.data.forEach(item => {
