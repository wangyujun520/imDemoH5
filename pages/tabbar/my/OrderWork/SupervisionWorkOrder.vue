	...
	export default {
		...
		methods: {
			InitData(){
				...
				this.$post(this.$url.Supervisor.GetOrders, requestData, {
					isLoading: false
				}).then(res => {
			...
			skipConstructionTask(item) {
				uni.navigateTo({
					url: '/pages/tabbar/my/OrderWork/ConstructionTask?order_id='+ item.order_id+'&order_surveyor_id='+item.id+'&module=surveyor'
				})
			},
