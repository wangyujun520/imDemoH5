...
<script>
	export default {
		onShow() {
			let search = uni.getStorageSync('EstateSearchData')
		...
		onNavigationBarSearchInputConfirmed(e) {
			let sear = uni.getStorageSync('EstateSearchData')
			let seas = sear + ',' + this.SearchData
			// 同步缓存
			uni.setStorage({
				key: 'EstateSearchData',
				data: seas,
				success: function() {
					console.log('success');
				}
			...
			uni.navigateTo({
				url: '/pages/tabbar/settle/HouseList?searchdata=' + SearchData
			});
		},
		data() {
			return {
		...
		methods: {
			...
			delLog() {
				...
				uni.removeStorageSync('EstateSearchData');
			}
		}
