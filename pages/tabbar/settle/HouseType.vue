<template>
	<view class="u-wrap">
		<view @tap="skipEstateSearch" class="u-search-box">
					...
					<view class="page-view">
						<view class="class-item">
							...
							<view class="item-container">
								<view @tap="skipPropertyDetails(item1)" class="thumb-box" v-for="(item1, index1) in item.list" :key="index1">
									<image class="item-menu-image" :src="item1.cover" mode=""></image>
									<view class="item-menu-name">{{item1.title}}</view>
								</view>
								<view v-if="item.list.length == 0" class="flex justify-center align-center" style="height: 75vh;width: 100%;">
			...
			// 初始化数据
			 InitData(){
				...
				this.$post(this.$url.Estate.GetDevs,{},{}).then(res=>{
					...
					res.data.forEach((item,index)=>{
						this.GetEsates(item,index)
					})
				})
			},
			GetEsates(item,index){
				//获取分类信息
				this.$post(this.$url.Estate.GetEsates,{
					dev_id: item.id,
					page_no: 1,
					page_size: 100
				},{}).then(res=>{
					res.data.list.forEach(item=>{
						this.tabbar[index].list.push(item);
					})
				})
			},
			getImg() {
