<template>
	<view class="content position-relative">
		<view class="flex flex-column" style="margin: 20rpx;">
			...
			<view class="flex flex-column bg-white mt-1 rounded" @tap="skipConstructionNode">
				<view class="flex align-center justify-between px-3" style="border: 0px solid red;height: 120rpx;">
					<view class="flex align-center">
						<view class="flex align-center justify-center mr-3"
							...
							<u-icon name="clock" v-if="DataList.work_node.status != 0" color="#2BA1DE" size="40">
							</u-icon>
							<u-icon name="checkmark-circle" v-else color="#02B1BE" size="40"></u-icon>

						</view>
						<view class="flex flex-column" style="font-size: 24rpx;">
							<text style="color: #939393;">施工节点</text>
							<!-- status!=0显示红色 -->
							<text v-if="DataList.work_node.status != 0" class="mt-1"
								style="color: red;">{{DataList.work_node.title}}-{{DataList.work_node.status_text}}</text>
							<text v-else class="mt-1"
								style="color: #02B1BE;">{{DataList.work_node.title}}-{{DataList.work_node.status_text}}</text>
			...
			<!-- <view class="flex flex-column p-2 bg-white mt-2" style="border-radius: 10rpx 10rpx 10rpx 10rpx;">
				...
				<view class="flex align-center my-1">
					<text style="font-size: 26rpx;color: #02B1BE;">已接受：0</text>
					<text style="font-size: 26rpx;color: #FF7100;" class="ml-3">待接受：12</text>
					<text style="font-size: 26rpx;color: #FF0000;" class="ml-3">已拒绝：1</text>
				</view>
				<text style="font-size: 26rpx;color: #FF9900;">等待客户确定人员</text>
			</view> -->
		...
		<view v-if="DataList.status == 1" class="position-fixed" style="height: 100rpx;width: 100%;bottom: 0;">
			<view class="flex bg-white" style="width: 100%;height: 100%;border-top: 1px solid rgba(187,187,187,0.2);">
				...
				<view class="flex align-center justify-end pr-2" style="width: 350rpx;">
					...
					<text @click="ArgOrder('2')" class="ml-3"
		...
		<view v-if="DataList.status == 2" class="position-fixed" style="height: 100rpx;width: 100%;bottom: 0;">
			<view class="flex bg-white" style="width: 100%;height: 100%;border-top: 1px solid rgba(187,187,187,0.2);">
				<view class="flex align-center pl-2" style="width: 400rpx;">
					<!-- <text @click="gotoIM" style="background-color: #FFFFFF;color: #2979FF;font-size: 26rpx;padding: 10rpx 40rpx;border-radius: 40rpx;border: 1px solid #2979FF;">
						联系设计师
...
<script>
	export default {
		...
		methods: {
			ArgOrder(state) {
				uni.showModal({
					...
					success: (res) => {
						if (res.confirm) {
							//0=接受；1=拒绝
							this.$post(this.$url.Worker.ArgOrder, {
								id: this.id,
								status: state
							}, {}).then(res => {
								// 成功
			...
			// 跳转到施工节点
			skipConstructionNode() {
				if (this.DataList.status != 1) {
					uni.navigateTo({
						url: '/pages/UserLive/ConstructionNodeDetail?id=' + this.DataList.work_node.catalog_id +
							'&title=' + this.DataList.work_node.title
					})
				}
			},
			// 跳转页面
			...
			InitData() {
				this.$post(this.$url.Worker.GetOrder, {
					id: this.id
				}, {
					isLoading: true,
					loadingTitle: '加载中...'
