<template>
	<view class="content">
		<view class="p-2">
			<view class="px-2 flex flex-column bg-white rounded">
				...
				<view class="flex py-2 px-3 my-2 justify-between align-center rounded"
					...
					<view class="flex align-center">
						<image class="rounded" :src="DataList.designer.header" style="width: 100rpx;height: 100rpx;">
						</image>
						<view class="flex flex-column ml-2">
							<view class="flex align-center">
								<text class="mr-1"
									style="font-size: 26rpx;color: #000;font-weight: bold;">{{DataList.designer.name}}</text>
								<text class="iconfont" style="font-size: 30rpx;color: #FF9900;">&#xe69d;</text>
							</view>
							<text class="mt-1" style="color: #000;font-size: 26rpx;font-weight: bold;">设计师总服务费</text>
						</view>
					</view>
			...
			<view class="bg-white flex flex-column my-2 p-2 rounded">
				<view @click="skipPaid(item)" v-for="(item,index) in DataList.money_list" :key="index"
					class="flex flex-column py-2" style="border-bottom: 1px solid rgba(187,187,187,0.2);">
					<view class="flex justify-between">
						...
						<view class="flex align-center">
							...
							<text v-if="item.ext_count>0" class="ml-1" style="color: #02B1BE;">({{item.ext_money}}+{{item.money}})</text>
						</view>
						<!-- <text :class="item.status==0?'yes-pay':'no-pay'" style="font-size: 26rpx;">￥{{item.ext_money}}</text> -->
					</view>
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">{{item.sub_title}}</text>
						<view class="flex align-center">
							<text v-if="item.ext_count>0" class="mr-1" style="color: #02B1BE;">+{{item.ext_count}}</text>
							<text :class="item.status==0?'yes-pay':'no-pay'" style="font-size: 26rpx;">{{item.status_text}}</text>
							<!-- <text @tap.stop="tuikuan(item)" class="ml-1" style="color: #FF9900;font-size: 26rpx;" v-if="item.status==0&&module=='user'">申请退款</text> -->
						</view>
					</view>
				</view>
				<!-- <view class="flex flex-column py-2" style="border-bottom: 1px solid rgba(187,187,187,0.2);">
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">第二笔费用支付</text>
						<text style="color: #FF0000;font-size: 26rpx;">￥1799.80</text>
					</view>
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">签订合同后支付40%</text>
						<text style="color: #FF9900;font-size: 26rpx;">待支付</text>
					</view>
				</view>
				<view class="flex flex-column py-2" style="border-bottom: 1px solid rgba(187,187,187,0.2);">
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">第三笔费用支付</text>
						<text style="color: #FF0000;font-size: 26rpx;">￥3599.60</text>
					</view>
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">签订合同后支付80%</text>
						<text style="color: #888;font-size: 26rpx;">未开始</text>
					</view>
				</view>
				<view class="flex flex-column py-2" style="border-bottom: 1px solid rgba(187,187,187,0.2);">
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">第四笔费用支付</text>
						<text style="color: #FF0000;font-size: 26rpx;">￥1799.80</text>
					</view>
					<view class="flex justify-between">
						<text style="font-size: 26rpx;color: #5B5B5B;">签订合同后支付100%</text>
						<text style="color: #888;font-size: 26rpx;">未开始</text>
					</view>
				</view> -->
			</view>
		...
		<!-- 立即支付 -->
		<view v-if="rankno==0&&node_status!=0&&ShowPay" class="position-fixed bg-white" style="width: 750rpx;height: 120rpx;bottom: 0;">
			<view class="flex align-center justify-between px-3" style="width: 100%;height: 120rpx;">
				...
				<view class="flex align-end">
					...
					<text class="ml-1" style="color: #666;font-size: 24rpx;margin-bottom: 10rpx;">{{DataList.paying.title}}</text>
				</view>
				<view class="flex align-center">
...
<script>
	export default {
		...
		onShow() {
			// this.ShowPopup = false;//支付框关闭
			this.InitData();
		},
		methods: {
			// 初始化数据
			InitData() {
				this.$post(this.$url.Task.GetPayOrder, {
			...
			skipPaid(item) {
				...
				// return
				uni.navigateTo({
					url:`/pages/ServiceOrder/Paid?id=${item.id}&title=支付信息&module=${this.module}&order_id=${this.order_id}&code=${item.code}&catalog=${item.catalog}`
				})
			},
			checkUserToRoom() {
