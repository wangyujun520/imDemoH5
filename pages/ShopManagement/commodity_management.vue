<template>
	<view class="content">
		...
		<view class="flex flex-column rounded m-3" @tap="skipProductDetails(item)" v-for="(item,index) in DataList"
			:key="item.id">
				...
				<view class="flex flex-column pl-3" style="border: 0px solid red;width: 450rpx;">
					...
					<view class="flex flex-wrap">
						<text v-for="tag in item.tags"
							style="color: #ff9900;border: 1px solid #ff9900;padding: 0rpx 20rpx;"
							class="mr-2 mt-2 rounded">{{tag}}</text>
					</view>
					<view class="flex align-center mt-2" style="color: red;">
						<text style="font-size: 26rpx;">￥</text>
						<text style="font-size: 30rpx;font-weight: bold;">{{item.price}}</text>
					</view>
				</view>
			</view>
			<view class="flex align-center justify-between bg-white px-3"
				style="height: 100rpx;border-top: 1px solid rgba(187, 187, 187, 0.2);">
				<view class="flex">
					<text style="font-size: 26rpx;">状态：</text>
					<text style="font-size: 26rpx;color: #F99A00;">{{item.status==0?'启用':'停用'}}</text>
				</view>
				<text @tap.stop="showMenu(item)" class="iconfont" style="color: #666;font-size: 40rpx;">&#xe6f3;</text>
			</view>
		</view>
		...
		<u-action-sheet :list="ActionSheetList" v-model="ActionSheetListShow" @click="UpdateMallGoods"></u-action-sheet>
...
<script>
	export default {
		...
		onNavigationBarButtonTap(e) {
			if (e.float == 'right') {
				uni.navigateTo({
					url: '/pages/ShopManagement/commodity_add'
				})
			}
		},
		data() {
		...
		methods: {
			showMenu(item) {
				this.tempData = item;
				if (item.status == 0) {
					this.ActionSheetList[0].text = '停用';
				} else {
					this.ActionSheetList[0].text = '启用';
				}
				this.ActionSheetListShow = true;
			},
			// 新增、更新商城商品基础信息
			UpdateMallGoods(index) {
				// 编辑
				if(index == 0){
					console.log(this.tempData.status)
					let status = 0;
					if (this.tempData.status == 0) {
						status = 1;
					} else {
						status = 0;
					}
					...
					this.$post(this.$url.Mall.UpdateMallGoods, requestData, {}).then(res => {
				...
				if(index == 1){
					// 删除商城商品
					this.$post(this.$url.Mall.DelMallGoods, {
						id:this.tempData.id
					}, {}).then(res => {
						this.$refs.uToast.show({
							title: '删除成功',
			...
			InitData() {
				let requestData = {
					status: this.status, //状态；0=启用；1=停用
					key: "", //关键字
					page_no: 1, //页码
					page_size: 10 //每页数量
				}
				this.$post(this.$url.Mall.GetMallGoods, requestData, {}).then(res => {
			...
			skipProductDetails(item) {
				uni.navigateTo({
					url: '/pages/ShopManagement/commodity_details?id=' + item.id
				})
			},
			change(index) {
				this.current = index;
