		...
		<view v-else class="flex align-start position-relative mb-3" :class="isself ? 'justify-end' : 'justify-start'">
			...
			<template v-if="!isself">
				...
				<view class="p-2 rounded" style="max-width:500rpx;" :class="labelClass" :style="labelStyle">
					...
					<!-- 位置信息 -->
					<view @click="openAddress(item)" class="flex flex-column bg-white" v-else-if="item.type === 'location'" style="width: 480rpx;">
						<!-- 地址 -->
				...
				<!-- 消息状态 -->
				<!-- <text class="text-danger" style="margin-top: 30rpx;font-size: 12px;margin-left: 10rpx;">未读</text> -->
			</template>
...
import $T from '@/components/free-ui-2/time.js';
...
export default {
	...
	data() {
		return {
			...
			ChatLongState:false,
			covers: [{
				latitude: 30.72028618706596,
				longitude: 104.07612928602427,
