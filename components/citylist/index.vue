	...
	export default{
		...
		methods:{
			...
			selectedCity(item){
				...
				prevPage.$vm.locationCity =  item;//修改上一页data里面的locationCity参数值
				// 返回上一个界面
				uni.navigateBack({
					delta:1
				})
			...
			location(){
				...
				let pages = getCurrentPages();  //获取所有页面栈实例列表
				let prevPage = pages[ pages.length - 2 ];  //上一页页面实例
				prevPage.$vm.locationCity =  this.locationCity;//修改上一页data里面的locationCity参数值
				// 返回上一个界面
				uni.navigateBack({
					delta:1
				})
		...
		mounted(){
			...
			uni.getLocation({
				...
				success: function (res) {
					...
					that.locationCity = res.address.city;
